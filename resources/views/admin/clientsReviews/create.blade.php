...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.clientsReview.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.clients-reviews.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ClientsReview::RATE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('rate') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="rate_{{ $key }}" name="rate" value="{{ $key }}" {{ old('rate', '') === (string) $key ? 'checked' : '' }} required>
